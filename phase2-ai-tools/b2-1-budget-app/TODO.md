...
## 확장 후보 (과제 요구 밖 · 착수 보류 2026-09-21)

> **B2-1 구현은 여기서 종료한다.** 아래는 과제 요구사항이 아니며, 제출에 영향이 없다.
> 2026-09-21 토론에서 나온 "안전성 교육이 한 겹에서 멈췄다"는 판단의 후속이다.
> 착수 시점: B2-2 이후 여유가 생겼을 때, 또는 Step 4 도메인 과제에서 필요해질 때.

| 후보 | 왜 | 현재 코드의 구멍 |
|---|---|---|
| 감사 추적 (append-only `audit.log`) | 제약 GMP 데이터 무결성(ALCOA+)의 핵심 요구 | `update`/`delete` 가 파일을 다시 써서 **이전 값이 사라진다** |
| `os.fsync` 추가 | `os.replace` 는 이름 교체만 원자적. 디스크 반영은 보장하지 않는다 | 교체 직후 전원 차단 시 유실 가능 |
| 파일 잠금 | 프로세스 2개 동시 실행 대비 | 같은 id 중복 채번, `rewrite` 와 `append` 충돌 시 유실 |
| `--json` 출력 옵션 | 사람이 읽는 표만 있고 기계가 읽을 출력이 없다 | 다른 프로그램·에이전트가 이 도구를 못 쓴다 |

**우선순위**: 감사 추적 1순위. 이것만 해도 동료평가에서 "도메인 관점으로 한 겹 더 봤다"가 된다.
